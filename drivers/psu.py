...
class PSUDataSource(abc.ABC):
    """Abstract interface for Power Supply Unit measurement log."""
    @abc.abstractmethod
...
class PSUCSVReader(PSUDataSource):
    def __init__(self, file_path: str, test_config: Dict[str, Any]):
        self.file_path = file_path
        self.stats = {"total_rows": 0, "malformed_rows": 0, "timestamp_gaps": 0}
        
        # 1. Zero Hardcoding: Dynamic Column & Type Extraction for PSU
        try:
            csv_config = test_config['data_sources']['power_supply']['formats']['csv']['columns']
            ...
            raise ValueError(f"CRITICAL: Missing required YAML config for PSU: {e}")
        ...
        with open(self.file_path, 'r', encoding='utf-8-sig') as f:
            ...
            for row_idx, row in enumerate(reader, start=2): 
                ...
                if None in row.values() or None in row.keys():
                    logger.warning(f"PSU Row {row_idx} malformed: incorrect field count. Skipping.")
                    self.stats["malformed_rows"] += 1
                    continue

                # PDF Constraint: Dynamic Type Conversion
                try:
                    parsed_row = {}
                    for col_name, cast_func in self.expected_columns.items():
                        parsed_row[col_name] = cast_func(row[col_name])
                except ValueError:
                    logger.warning(f"PSU Row {row_idx} malformed: invalid type conversion. Skipping.")
                    self.stats["malformed_rows"] += 1
                    continue
                ...
                if current_ts <= last_timestamp:
                    logger.warning(f"PSU Timestamp issue at row {row_idx}: {current_ts} follows {last_timestamp}.")
